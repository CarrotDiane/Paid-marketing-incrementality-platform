USE ROLE IC_DS_ROLE;
USE DATABASE INSTADATA;

create or replace table sandbox_db.dianedou.OTT_synthetic_control_cohort_vprod_temp as

  with visit_base as (
      SELECT distinct oad.omni_user_id
      , oad.visit_token
      , date_trunc('day', oad.omni_date_time_pt::date) as visit_date
      , case when paid_channel = 'OTT' then 1 else 0 end as exp_flag --exp_flag = 0; never exposed to paid channel = SEM
      , case when visit_date between $start_date and $end_date then 1 else 0 end as visit_flag --visit_flag = 1; visited during the assignment window
      FROM analysts.order_attribution_daily  oad
      WHERE omni_date_time_pt::date between $visit_start and $end_date
    )

  , control_cohort as (
    select vb.*
    from visit_base vb
    qualify max(exp_flag) over (partition by omni_user_id) = 0
    and max(visit_flag) over (partition by omni_user_id) = 1
    )

  , control_cohort_base as (
    select * from control_cohort
    where  visit_date between $start_date and $end_date
    qualify row_number() over (partition by omni_user_id order by visit_date) = 1 --dedup_rank
    )

    select visit_date as created_at_pt
    , date_trunc('week', visit_date) as wk_ref
    , $start_date as cohort
    , omni_user_id as user_id
    , 0 as variant
    from (select * from control_cohort_base sample (15)) base
;

----treatment cohorts
create or replace table sandbox_db.dianedou.OTT_synthetic_treatment_cohort_vprod_temp  as
  with visit_base as (
      SELECT distinct oad.omni_user_id
      , oad.visit_token
      , date_trunc('day', oad.omni_date_time_pt::date) as visit_date
      , row_number() over (partition by omni_user_id order by oad.omni_date_time_pt) as dedup_rank
      FROM  analysts.order_attribution_daily oad
      WHERE omni_date_time_pt::date between $start_date and $end_date and paid_channel = 'OTT'
    )

  , treatment_base as (select visit_date as created_at_pt
    , date_trunc('week', visit_date) as wk_ref
    , $start_date as cohort
    , omni_user_id as user_id
    , 1 as variant
...
select * from treatment_base sample(10);


create or replace table sandbox_db.dianedou.OTT_synthetic_existing_user_base_table_vprod_temp as
with base as
  (select * from sandbox_db.dianedou.OTT_synthetic_control_cohort_vprod_temp
  union all
  select * from sandbox_db.dianedou.OTT_synthetic_treatment_cohort_vprod_temp)

select t.*
, case when u.activation_date_pt < t.created_at_pt then 'existing' else 'new' end as user_type
from base t
left join dwh.dim_user u on u.user_id = t.user_id
where user_type = 'existing'
;
...
set user_base_table = 'sandbox_db.dianedou.OTT_synthetic_existing_user_base_table_vprod_temp';


create or replace table sandbox_db.dianedou.base_features_OTT_vprod_temp as
...
, fuga as (  --- save running time from querying fuga
  select fuga.* from  instadata.dwh.fact_user_growth_accounting fuga
...
create or replace table sandbox_db.dianedou.express_past_express_status_OTT_vprod_temp as
...
create or replace table sandbox_db.dianedou.shopping_OTT_vprod_temp as
...
CREATE OR REPLACE table sandbox_db.dianedou.itemattributes_OTT_vprod_temp AS
SELECT
    base.user_id,
    base.created_at_pt,
...
CREATE OR REPLACE table sandbox_db.dianedou.OTT_zip_median_income as
...
CREATE OR REPLACE table sandbox_db.dianedou.OTT_matching_attributes_vprod_temp as
...
income.median_income as zip_median_income,
...
from sandbox_db.dianedou.base_features_OTT_vprod_temp base
left join sandbox_db.dianedou.shopping_OTT_vprod_temp shop  on base.user_id = shop.user_id  and base.created_at_pt = shop.created_at_pt---and base.reference_month = shop.reference_month
left join sandbox_db.dianedou.itemattributes_OTT_vprod_temp item  on base.user_id = item.user_id  and base.created_at_pt = item.created_at_pt--and base.reference_month = item.reference_month
left join sandbox_db.dianedou.express_past_express_status_OTT_vprod_temp Xp on base.user_id = Xp.user_id and base.created_at_pt = Xp.created_at_pt--and base.reference_month = Xp.reference_month
left join sandbox_db.dianedou.OTT_zip_median_income income on base.user_id = income.user_id
;
